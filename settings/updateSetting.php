<?php
  ...
  if(isset($_POST["setting_id"])) { 
    ...
    $query = $pdo->prepare("UPDATE settings SET nama_masjid=?,alamat_masjid=?,running_teks=?,kecepatan_teks=?,durasi_slide=?,koreksi_hijriah=?,durasi_adzan=?,iqomah_shubuh=?,iqomah_dzuhur=?,iqomah_ashar=?,iqomah_maghrib=?,iqomah_isya=?,durasi_shubuh=?,durasi_syuruq=?,durasi_dzuhur=?,durasi_ashar=?,durasi_maghrib=?,durasi_isya=?,garis_lintang=?,garis_bujur=?,zona_waktu=?,metode_perhitungan=? WHERE setting_id=?");    
    $query->execute([$nama_masjid,$alamat_masjid,$running_teks,$kecepatan_teks,$durasi_slide,$koreksi_hijriah,$durasi_adzan,$iqomah_shubuh,$iqomah_dzuhur,$iqomah_ashar,$iqomah_maghrib,$iqomah_isya,$durasi_shubuh,$durasi_syuruq,$durasi_dzuhur,$durasi_ashar,$durasi_maghrib,$durasi_isya,$garis_lintang,$garis_bujur,$zona_waktu,$metode_perhitungan,$_POST['setting_id']]); 
